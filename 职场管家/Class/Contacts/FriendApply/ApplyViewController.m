    ...
    if(self.dataSource.count > indexPath.row)
    {
        ...
        if (entity) {
            ...
            else if (applyStyle == ApplyStyleJoinGroup)
            {
                ...
                Contact *contact = [ERPCache getContact:entity.applicantUsername];
                cell.titleLabel.text = contact.nickname;
                cell.headerImageView.image = contact.avatar;
            }
            NSString *reason = [entity.reason stringByReplacingOccurrencesOfString:@"-" withString:@"@"];
            cell.contentLabel.text = reason;
        }
    }
//    if (self.dataSource.count>indexPath.row) {
//        ApplyEntity *entityOne = [self.dataSource objectAtIndex:indexPath.row-1];
//        ApplyEntity *entityTwo = [self.dataSource objectAtIndex:indexPath.row];
//        if ([entityOne.applicantUsername isEqualToString:entityTwo.applicantUsername]) {
//            [[[ApplyViewController shareController] dataSource] removeLastObject];
//        }
//    }
...
//             contact.username = entity.applicantUsername;
//             BmobQuery *bquery = [BmobQuery queryWithClassName:@"_User"];
//             [bquery whereKey:@"username" equalTo:contact.username];
//             [bquery findObjectsInBackgroundWithBlock:^(NSArray *array, NSError *error) {
//                 if (array.count>0) {
//                     for (BmobObject *obj in array) {
//                         if ([[obj objectForKey:@"username"] isEqualToString:contact.username]) {
...
-(void)viewDidAppear:(BOOL)animated{
    self.navigationItem.leftBarButtonItem = [UIBarButtonItem itemWithTarget:self action:@selector(back) image:@"left32" highImage:@"left32"];
}
@end
