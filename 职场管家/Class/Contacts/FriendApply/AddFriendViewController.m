...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    Contact *contact = [ERPCache getContact:[self.dataSource objectAtIndex:indexPath.row]];
    cell.imageView.image = contact.avatar;
    cell.textLabel.text = [[self.dataSource objectAtIndex:indexPath.row] stringByReplacingOccurrencesOfString:@"-" withString:@"@"];
...
#warning 以下代码为测试代码，默认用户体系中有一个符合要求的同名用户
        ...
        NSString *username = [_textField.text stringByReplacingOccurrencesOfString:@"@" withString:@"-"];
        //判断是否已发来申请
        ...
        if (applyArray && [applyArray count] > 0) {
            for (ApplyEntity *entity in applyArray) {
                ...
                if (!isGroup && [entity.applicantUsername isEqualToString:username]) {
                    NSString *str = [NSString stringWithFormat:NSLocalizedString(@"friend.repeatInvite", @"%@ have sent the application to you"), username];
        ...
        Contact *contact =[ERPCache getContact:username];
        if ([contact.username isEqualToString:username]) {
            [self.dataSource removeAllObjects];
            [self.dataSource addObject:username];
            [self.tableView reloadData];
        }else{
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:NSLocalizedString(@"prompt", @"Prompt") message:@"您搜索的好友不存在！！！" delegate:nil cancelButtonTitle:NSLocalizedString(@"ok", @"OK") otherButtonTitles:nil, nil];
            [alertView show];
        }
