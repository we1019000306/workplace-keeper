...
- (void)viewDidLoad {
    ...
    [ERPCache openDBWithObjectId:buser.objectId];
...
//        if (profileEntity) {
            Contact *contact = [ERPCache getContact:model.buddy.username];
        if (contact.avatar) {
            model.avatarImage = contact.avatar;
        }else{
            model.avatarImage = [UIImage imageNamed:@"avatarDefault1@2x.png"];
        }
            model.avatarURLPath = contact.avatarURL;
...
- (void)_sortDataArray:(NSArray *)buddyList
{
    ...
    for (EMBuddy *buddy in contactsSource) {
        ...
        if (model) {
            Contact *contact = [ERPCache getContact:buddy.username];
            
           
            if ([contact.username isEqualToString:model.buddy.username]) {
                model.nickname = contact.nickname;
            }else{
...
//                NSArray *localBuddyList = [ERPCache getContact];
//                [self.contactsSource addObjectsFromArray:localBuddyList];
