...
@interface ConversationListController ()<EaseConversationListViewControllerDelegate, EaseConversationListViewControllerDataSource,UISearchDisplayDelegate, UISearchBarDelegate,UIGestureRecognizerDelegate>
                                    ...
                                    modelForConversation:(EMConversation *)conversation
{
    ...
    if (model.conversation.conversationType == eConversationTypeChat) {
        if ([[RobotManager sharedInstance] isRobotWithUsername:conversation.chatter]) {
            ...
            contact = [ERPCache getContact:conversation.chatter];
            if ([contact.username isEqualToString:conversation.chatter]) {
                model.title = contact.nickname;
            }else{
                model.title = conversation.chatter;
            }

            if (contact.avatar) {
                model.avatarImage = contact.avatar;
            }else{
                model.avatarImage = [UIImage imageNamed:@"user@2x.png"];
            }
        }
...
- (void)cellLongPressAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UIAlertAction *report = [UIAlertAction actionWithTitle:@"举报" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action) {
         UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"举报成功！！！" message:@"若该账号发布违法信息，管理员将马上采取停权措施！！！" preferredStyle:(UIAlertControllerStyleAlert)];
...
-(void)longPressToDo:(UILongPressGestureRecognizer *)gesture
{
    if(gesture.state == UIGestureRecognizerStateBegan)
    {
        ...
        if(indexPath == nil){ return ;}
            ...
            [self cellLongPressAtIndexPath:indexPath];
        }
    }
}
@end
